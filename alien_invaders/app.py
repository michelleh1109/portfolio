...
class Invaders(GameApp):
    ...
    # Attribute _player_won: status of whether player has won 
    # Invariant: _player_won is boolean True or False
    ...
    def start(self):
        ...
        self._wave = None
        self.lastkeys = 0
        self._text = GLabel(text="Press 'S' to Play", font_name='Arcade.ttf', 
        font_size=FONT_SIZE, x=GAME_WIDTH/2, y=GAME_HEIGHT/2)
    ...
    def update(self,dt):
        ...
        if self._state == STATE_ACTIVE:
            ...
            if self._wave.get_player_win():
                self._player_won = True
                self._state= STATE_COMPLETE
            elif self._wave.get_player_lose():
                self._player_won = False
                self._state= STATE_COMPLETE
            ...
            if self._wave.getLives()>=0:
                self._text = GLabel(text="Press 'S' to Continue.", font_name='Arcade.ttf', 
                font_size=FONT_SIZE, x=GAME_WIDTH/2, y=GAME_HEIGHT/2)
                self._change_STATE_PAUSED()
            else:
                self._state = STATE_COMPLETE
        elif self._state == STATE_COMPLETE:
            if self._player_won:
                self._text = GLabel(text="You Beat the Aliens!", 
                font_name='Arcade.ttf', font_size=FONT_SIZE, x=GAME_WIDTH/2, y=GAME_HEIGHT/2)
            else:
                self._text = GLabel(text="Better Luck Next Time", font_name='Arcade.ttf', 
                    font_size=FONT_SIZE, x=GAME_WIDTH/2, y=GAME_HEIGHT/2)
    ...
    def _change_STATE_INACTIVE(self):
        """
        Helper function for STATE_INACTIVE.

        Checks if 's' is the first key pressed. If so, it switches state to 
        STATE_NEWWAVE.
        """
        change = self.lastkeys == 0 and self.input.is_key_down('s')
        if change:
            self._text=None # changing welcome message
            self._state = STATE_NEWWAVE
            self._change_STATE_NEWWAVE()
            self.lastkeys = 1

    def _change_STATE_NEWWAVE(self):
        """
        Helper function for STATE_NEWWAVE

        Creates wave object and changes state to STATE_ACTIVE after one animation frame.
        """
        self._wave = Wave()
        self._state = 2

    def _change_STATE_PAUSED(self):
        """
